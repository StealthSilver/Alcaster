    ...
    <section className="relative py-20 sm:py-28">
      <Container>
        ...
        <motion.div
          ...
          className="mx-auto mt-14 grid max-w-4xl gap-6 sm:grid-cols-[1fr_auto_1fr] sm:items-stretch sm:gap-8"
        >
          <motion.div variants={fadeUp} className="flex flex-col">
            <p className="mb-4 text-[11px] font-medium uppercase tracking-[0.18em] text-white/35">
              Traditional Monitoring
            </p>
            <div className="flex flex-1 flex-col gap-2 rounded-xl border border-white/[0.08] bg-white/[0.02] p-5">
              {TRADITIONAL.map((item) => (
                <div
                  key={item}
                  className="rounded-lg border border-white/[0.06] bg-[#010609]/50 px-4 py-3 text-sm text-white/45"
                >
                  {item}
                </div>
              ))}
          ...
          <motion.div variants={fadeUp} className="flex flex-col">
            ...
            <div
              className={cn(
                "relative flex flex-1 flex-col gap-2 overflow-hidden rounded-xl border border-[#e6740a]/25 bg-[#e6740a]/[0.06] p-5",
              )}
            >
              <div
              ...
              {ALCASTER.map((item, i) => (
                <div
                  key={item}
                  className={cn(
                    "relative rounded-lg border px-4 py-3 text-sm font-medium",
                    i === 0
